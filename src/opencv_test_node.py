...
class ImageConverter:
    ...
    def hay_objeto(self):
        if self.scan:
            for r in self.scan.ranges[355:365]:
    ...
    def random_walk(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_hydrant or self.is_dumpster:
                ...
                # print("Objeto en la distancia limite: {:.3f}".format(self.scan.ranges[360]))
            else:
    ...
    def go_to_object(self):
        ...
        if self.hay_objeto():
            ...
            wait = now + timedelta(seconds=2.0)
            while datetime.now().second < wait.second:
                self.vel_msg.linear.x = 0
    ...
    def find_hydrant(self, cv_image, hsv_img, color_lower, color_upper):
        ...
        if len(contours) > 0:
            ...
            if w > 5 and h > 40 and not self.hay_objeto():
                ...
                if (x or y) and not self.is_dumpster:
    ...
    def find_dumpster(self, cv_image, hsv_img, color_lower, color_upper):
        ...
        if len(contours) > 0:
            ...
            if w > 70 and h > 50 and not self.hay_objeto():
                ...
                if (x or y) and not self.is_hydrant:
                    self.center_husky(x, y, w, h)
